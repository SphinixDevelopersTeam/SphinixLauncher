...
function showMainUI(data){
    ...
    if(!isDev){
        loggerAutoUpdater.log('Инициализация..')
...
function showFatalStartupError(){
    setTimeout(() => {
        $('#loadingContainer').fadeOut(250, () => {
            ...
            setOverlayContent(
                'Фатальная ошибка: невозможно загрузить файл настройки',
                'Не удалось установить соединение с нашими серверами для загрузки файла настроек. Нет локальных копий, доступных для загрузки. <br><br>Файл настроек — это важный файл, содержащий самую свежую информацию о серверах. Без него лаунчер не запустится. Убедитесь, что вы подключены к Интернету и перезапустите приложение.',
                'Закрыть'
            )
            setOverlayHandler(() => {
    ...
    if(selectedAcc != null){
        ...
        if(!val){
            ...
            setOverlayContent(
                ...
                `Нам не удалось обновить логин для <strong>${selectedAcc.displayName}</strong>. Пожалуйста, ${accLen > 0 ? 'выберите другую учетную запись или' : ''} войдите снова.`,
                'Вход',
