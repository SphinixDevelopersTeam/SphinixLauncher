...
const msftLoginLogger = LoggerUtil.getLogger('Логин при помощи Microsoft')
const msftLogoutLogger = LoggerUtil.getLogger('Выйти из уетной записи Microsoft')
    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        switchView(getCurrentView(), viewOnClose, 500, 500, () => {
            ...
            if(arguments_[1] === MSFT_ERROR.NOT_FINISHED) {
                // User cancelled.
                msftLoginLogger.info('Вход отменен пользователем.')
                return
            }
            ...
            setOverlayContent(
                'Что-то пошло не так',
                'Ошибка аутентификации Microsoft. Пожалуйста, попробуйте еще раз.',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
        ...
        if (Object.prototype.hasOwnProperty.call(queryMap, 'error')) {
            ...
            msftLoginLogger.info('Получен authCode, продолжается аутентификация.')
            ...
            AuthManager.addMicrosoftAccount(authCode).then(value => {
                ...
                .catch((displayableError) => {
                    ...
                    if(isDisplayableError(displayableError)) {
                        msftLoginLogger.error('Ошибка при входе.', displayableError)
                        actualDisplayableError = displayableError
                    } else {
                        // Uh oh.
                        msftLoginLogger.error('Необработанная ошибка при входе в систему.', displayableError)
                        actualDisplayableError = {
                            title: 'Неизвестная ошибка при входе в систему',
                            desc: 'Произошла неизвестная ошибка. Подробности смотрите в консоли.'
                        }
                    }
        ...
        val.onclick = (e) => {
            ...
            for(let i=0; i<selectBtns.length; i++){
                if(selectBtns[i].hasAttribute('selected')){
                    ...
                    selectBtns[i].innerHTML = 'Выбранный'
                }
            }
            val.setAttribute('selected', '')
            val.innerHTML = 'Выбранный &#10004;'
    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Предупреждение<br>Это ваш единственный аккаунт',
                    'Чтобы использовать лаунчер, вы должны войти хотя бы в одну учетную запись. После этого вам потребуется снова войти в систему.<br><br>Вы уверены, что хотите выйти?',
                    'Да, выйти',
                    'Отмена'
                )
                setOverlayHandler(() => {
    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
            ...
            if(arguments_.length > 1 && arguments_[1] === MSFT_ERROR.NOT_FINISHED) {
                // User cancelled.
                msftLogoutLogger.info('Выход отменен пользователем.')
                return
            }
            ...
            setOverlayContent(
                'Что-то пошло не так',
                'Ошибка выхода из Microsoft. Пожалуйста, попробуйте еще раз.',
                'OK'
            )
            setOverlayHandler(() => {
                toggleOverlay(false)
        ...
        msftLogoutLogger.info('Выход успешен. uuid:', uuid)
            ...
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">Имя прользователя</div>
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Выбранный &#10004;' : '>Выбрать'}</button>
                    <div class="settingsAuthAccountWrapper">
                        <button class="settingsAuthAccountLogOut">Выход</button>
                    </div>
                </div>
            </div>
        </div>`
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    `Не удалось удалить<br>добаленный мод ${fullName}`,
                    'Убедитесь, что файл не используется, и повторите попытку.',
                    'Окей'
                )
        ...
        if(dropinUI != null){
            ...
            if(DropinModUtil.isDropinModEnabled(dropin.fullName) != dropinUIEnabled){
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            'Не удалось переключить<br>один или несколько добавленных модов',
                            err.message,
                            'Окей'
                        )
                        setOverlayHandler(null)
        ...
        <div class="serverListingDetails">
            ...
            <div class="serverListingInfo">
                ...
                ${serv.isMainServer() ? `<div class="serverListingStarWrapper">
                    ...
                    <span class="serverListingStarTooltip">Основной сервер</span>
                </div>` : ''}
            </div>
        </div>
    `
...
settingsMinRAMRange.onchange = (e) => {
    ...
    if(sMaxV < sMinV){
        ...
        settingsMaxRAMLabel.innerHTML = sMinV.toFixed(1) + 'Гб'
    }

    // Update label
    settingsMinRAMLabel.innerHTML = sMinV.toFixed(1) + 'Гб'
}
...
settingsMaxRAMRange.onchange = (e) => {
    ...
    if(sMaxV < sMinV){
        ...
        settingsMinRAMLabel.innerHTML = sMaxV.toFixed(1) + 'Гб'
    }
    settingsMaxRAMLabel.innerHTML = sMaxV.toFixed(1) + 'Гб'
}
...
function populateMemoryStatus(){
    settingsMemoryTotal.innerHTML = Number((os.totalmem()-1000000000)/1000000000).toFixed(1) + 'Гб'
    settingsMemoryAvail.innerHTML = Number(os.freemem()/1000000000).toFixed(1) + 'Гб'
}
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Выбранна: Java ${v.version.major} Обновление ${v.version.update} (x${v.arch})${vendor}`
            } else {
                settingsJavaExecDetails.innerHTML = `Выбранна: Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})${vendor}`
            }
        } else {
            settingsJavaExecDetails.innerHTML = 'Неверный выбор'
        }
    })
}
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        titleElement.innerHTML = 'Предварительный выпуск'
        titleElement.style.color = '#ff886d'
        checkElement.style.background = '#ff886d'
    } else {
        titleElement.innerHTML = 'Стабильная версия'
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/SphinixDevelopersTeam/SphinixLauncher/releases.atom',
        success: (data) => {
        ...
        settingsAboutChangelogText.innerHTML = 'Не удалось загрузить примечания к выпуску.'
    })
}
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        settingsUpdateTitle.innerHTML = `New ${isPrerelease(data.version) ? 'Предварительный выпуск' : 'Стабильная версия'} готов к установке`
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Загрузите с GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Закройте панель запуска и запустите dmg для обновления.</span>', false, () => {
                shell.openExternal(data.darwindownload)
            })
        } else {
            settingsUpdateButtonStatus('Загрузка..', true)
        }
    } else {
        settingsUpdateTitle.innerHTML = 'Вы используете последнюю версию'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Проверить наличие обновлений', false, () => {
            if(!isDev){
